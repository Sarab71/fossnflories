...
const MenFrames = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('/api/productsByCategory?categories=MenFrames,UnisexFrames');
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Men Frames</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {products.length > 0 ? (
          products.map((product) => (
            <div
              key={product._id}
              className="border rounded-md p-4 shadow hover:shadow-lg transition cursor-pointer" // Add cursor-pointer
